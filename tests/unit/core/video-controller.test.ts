...
describe('VideoController', () => {
  ...
  it('VideoController should initialize speed using adjustSpeed method', async () => {
    ...
    let adjustSpeedParams: { video: HTMLMediaElement; value: number; options: Record<string, unknown> } | null = null;
    const originalAdjustSpeed = actionHandler.adjustSpeed;
    actionHandler.adjustSpeed = function (
  ...
  it('VideoController should properly setup event handlers', async () => {
    ...
    const addedListeners: Array<{ type: string; listener: EventListener; options?: unknown }> = [];
    const originalAddEventListener = mockVideo.addEventListener;
    mockVideo.addEventListener = function (
      type: string,
      listener: EventListenerOrEventListenerObject,
      options?: boolean | AddEventListenerOptions
    ) {
      addedListeners.push({ type, listener: listener as EventListener, options });
  ...
  it('VideoController should handle media events correctly', async () => {
    ...
    const adjustSpeedCalls: Array<{ video: HTMLMediaElement; value: number; options: unknown }> = [];
    const originalAdjustSpeed = actionHandler.adjustSpeed;
    actionHandler.adjustSpeed = function (
      video: HTMLMediaElement,
      value: number,
