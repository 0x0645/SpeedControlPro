...
runner.test('DEFAULT_SETTINGS should have keyBindings populated', () => {
  ...
  assert.equal(
    defaults.keyBindings.length > 0,
    true,
    'DEFAULT_SETTINGS.keyBindings should not be empty'
  );
...
runner.test('DEFAULT_SETTINGS keyBindings should have proper structure', () => {
  ...
  defaults.keyBindings.forEach((binding, index) => {
    ...
    assert.equal(
      typeof binding.predefined,
      'boolean',
      `Binding ${index}: predefined should be boolean`
    );
  });
});
...
runner.test('Fresh install should not require first-time initialization', async () => {
  ...
  assert.equal(
    config.settings.keyBindings.length > 0,
    true,
    'Should have default keyBindings on fresh install'
  );
...
runner.test('Should handle existing keyBindings in storage', async () => {
  ...
  assert.equal(
    config2.settings.keyBindings.length >= existingBindings.length,
    true,
...
runner.test('Should save keyBindings to storage correctly', async () => {
  ...
  assert.equal(
    config2.settings.keyBindings.length >= customBindings.length,
    true,
...
runner.test('Should never lose all keyboard shortcuts', async () => {
  ...
  assert.equal(
    config.settings.keyBindings && config.settings.keyBindings.length > 0,
    true,
